...
function generateMailToLink(subject, body, email = "info@nestlejobs.cc") {
    return `mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

function submitViaGmail() {
    const formData = gatherFormData();
    const subject = `${formData["Full Name"]} - Ads Specialist Application`;
    const body = Object.entries(formData)
        .map(([key, value]) => `${key === 'Cv Link' ? 'CURRICULUM VITAE (CV)' : key.toUpperCase()}: ${value}`)
        .join("\n\n");

    const mailtoLink = `https://mail.google.com/mail/u/0/?fs=1&to=info@nestlejobs.cc&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}&tf=cm`;
